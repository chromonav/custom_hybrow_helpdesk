...
**Step 3**: The site [http://custom_hybrow_helpdesk.localhost:8000/helpdesk](http://custom_hybrow_helpdesk.localhost:8000/helpdesk) should now be available. The default credentials are:
...
1. In a separate terminal window, create a new site by running `bench new-site custom_hybrow_helpdesk.test`
1. Map your site to localhost with the command `bench --site custom_hybrow_helpdesk.test add-to-hosts`
1. Get the Helpdesk app. Run `bench get-app https://github.com/frappe/helpdesk`
1. Run `bench --site custom_hybrow_helpdesk.test install-app helpdesk`.
1. Run `bench build --app helpdesk`
1. Now open the URL `http://custom_hybrow_helpdesk.test:8000/helpdesk` in your browser, you should see the app running
...
1. Open a new terminal session and cd into `frappe-bench/apps/helpdesk/desk`, and run the following commands:
    ```
    yarn install
    yarn dev or yarn dev --host custom_hybrow_helpdesk.test
    ```
1. Now, you can access the site on vite dev server at `http://custom_hybrow_helpdesk.test:8080`
